...
let viewportHeight = config.viewportHeight || 1080;
...
console.log(inputValues);
  ...
  (async () => {
    ...
    for(b of browsers){
      ...
      screenshots_directory_error = `${basePath}/screenshotsError`;
      ...
      if (!fs.existsSync(screenshots_directory_error)){
        console.log("Crear directorio error")
        fs.mkdirSync(screenshots_directory_error, { recursive: true });
      }
      else{
        clean(screenshots_directory_error)
      }
...
async function interactWithObject(object, page, currentState, interactionNumber, link){
  if(object.type === 'input' || object.type === 'button'){
